...
/**
 * 解析可读路径 — 从存储组的所有镜像中找到文件的实际路径
 * 内部处理镜像选择（随机负载均衡），业务层无需关心物理路径
 * @param {string} relativePath - 相对路径如 2026/06/12/uuid.enc
 * @param {number} groupId - 存储组ID
 * @returns {string|null} 第一个可用的完整路径，全部不可用返回 null
 */
function resolveReadPath(relativePath, groupId) {
  var StoragePool = require('./db').StoragePool;
  var allPools = StoragePool.listAll().filter(function(p) {
    return p.group_id === groupId && p.status !== 'deleted' && p.status !== 'error';
  });
...
/**
 * 解析所有可读路径 — 返回所有镜像中存在的完整路径列表（用于容错重试）
 * @param {string} relativePath - 相对路径
 * @param {number} groupId - 存储组ID
 * @returns {string[]} 所有可用的完整路径
 */
function resolveAllReadPaths(relativePath, groupId) {
  var StoragePool = require('./db').StoragePool;
  var allPools = StoragePool.listAll().filter(function(p) {
    return p.group_id === groupId && p.status !== 'deleted' && p.status !== 'error';
  });
...
module.exports = {
  ...
  resolveAllReadPaths: resolveAllReadPaths,
