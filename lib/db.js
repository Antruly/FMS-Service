...
var TrafficLog = {
  ...
  log: function(userId, guestIp, actionType, fileId, fileName, fileSize, bytesCount) {
    var now = new Date().toISOString();
    console.log('[TrafficLog.log] inserting:', { userId, actionType, fileName, bytesCount, now });
    run(
    ...
    console.log('[TrafficLog.log] insert done');
  },

  // 批量插入（高效）
  logBatch: function(records) {
    console.log('[TrafficLog.logBatch] called with', records.length, 'records');
    records.forEach(function(r) {
      TrafficLog.log(r.user_id, r.guest_ip, r.action_type, r.file_id, r.file_name, r.file_size, r.bytes_count);
    });
    console.log('[TrafficLog.logBatch] completed');
  },
