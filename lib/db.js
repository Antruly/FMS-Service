    ...
    try {
      ...
      safeCreateTable(db, `
        CREATE TABLE IF NOT EXISTS virtual_files (
          ...
          reference_source_id INTEGER DEFAULT 0,
      ...
      safeAddColumn(db, 'deleted_files', 'storage_id INTEGER DEFAULT 0');
      ...
      safeAddColumn(db, 'shares', "status TEXT DEFAULT 'active'");
      db.run("UPDATE shares SET status = 'active' WHERE status IS NULL");
      ...
      // 迁移：禁用/启用 + 最后访问时间（v1.1）
      safeAddColumn(db, 'shares', 'disabled INTEGER DEFAULT 0');
      safeAddColumn(db, 'shares', 'disabled_at TEXT');
      safeAddColumn(db, 'shares', 'disabled_by TEXT');
      safeAddColumn(db, 'shares', 'last_accessed_at TEXT');
      db.run("UPDATE shares SET disabled = 0 WHERE disabled IS NULL");
      ...
      safeCreateTable(db, `
        CREATE TABLE IF NOT EXISTS webdav_links (
          ...
          target_name TEXT NOT NULL,
          target_type TEXT DEFAULT 'public',
      ...
      safeAddColumn(db, 'webdav_links', "status TEXT DEFAULT 'active'");
      safeAddColumn(db, 'webdav_links', "link_name TEXT DEFAULT ''");
      db.run("UPDATE webdav_links SET status = 'active' WHERE status IS NULL");

      // 迁移：禁用/启用（v1.1）
      safeAddColumn(db, 'webdav_links', 'disabled INTEGER DEFAULT 0');
      safeAddColumn(db, 'webdav_links', 'disabled_at TEXT');
      safeAddColumn(db, 'webdav_links', 'disabled_by TEXT');
      db.run("UPDATE webdav_links SET disabled = 0 WHERE disabled IS NULL");
      ...
      // 默认规则初始化：表为空时插入默认配置（对应当前 server.js 硬编码值）
      try {
        var existingRules = db.exec('SELECT COUNT(*) as c FROM rate_limit_rules');
        if (!existingRules.length || !existingRules[0].values.length || existingRules[0].values[0][0] === 0) {
          var nowStr = new Date().toISOString();
          // 已登录用户：1000次/60秒 → 封禁60秒
          db.run("INSERT INTO rate_limit_rules (user_type, window_seconds, max_requests, ban_duration_seconds, sort_order, created_at, updated_at) VALUES ('authenticated', 60, 1000, 60, 1, ?, ?)", [nowStr, nowStr]);
          // 未登录用户：4级递进
          db.run("INSERT INTO rate_limit_rules (user_type, window_seconds, max_requests, ban_duration_seconds, sort_order, created_at, updated_at) VALUES ('anonymous', 60, 60, 60, 1, ?, ?)", [nowStr, nowStr]);
          db.run("INSERT INTO rate_limit_rules (user_type, window_seconds, max_requests, ban_duration_seconds, sort_order, created_at, updated_at) VALUES ('anonymous', 60, 120, 3600, 2, ?, ?)", [nowStr, nowStr]);
          db.run("INSERT INTO rate_limit_rules (user_type, window_seconds, max_requests, ban_duration_seconds, sort_order, created_at, updated_at) VALUES ('anonymous', 60, 200, 604800, 3, ?, ?)", [nowStr, nowStr]);
          db.run("INSERT INTO rate_limit_rules (user_type, window_seconds, max_requests, ban_duration_seconds, sort_order, created_at, updated_at) VALUES ('anonymous', 60, 400, 0, 4, ?, ?)", [nowStr, nowStr]);
          // 默认白名单
          db.run("INSERT INTO rate_limit_whitelist (path, description, created_at) VALUES ('/api/version/latest', '版本检查接口', ?)", [nowStr]);
          db.run("INSERT INTO rate_limit_whitelist (path, description, created_at) VALUES ('/favicon.ico', '网站图标', ?)", [nowStr]);
        }
      } catch (e) { /* 静默忽略默认数据插入错误 */ }
      ...
      try {
        ...
        db.run('CREATE INDEX IF NOT EXISTS idx_rate_limit_user_type ON rate_limit_rules(user_type)');
        db.run('CREATE INDEX IF NOT EXISTS idx_rate_limit_whitelist_path ON rate_limit_whitelist(path)');
      } catch (e) {}

              // 迁移：traffic_logs 新增 traffic_category 列（区分 request / file_transfer）
              safeAddColumn(db, 'traffic_logs', 'traffic_category TEXT DEFAULT \'file_transfer\'');
              try {
                db.run('CREATE INDEX IF NOT EXISTS idx_traffic_logs_category ON traffic_logs(traffic_category)');
              } catch (e) {}
      ...
      // 默认存储组自动创建默认镜像路径（组0），用户注册后即可直接上传
      try {
        var defaultPool = db.exec("SELECT id FROM storage_pools WHERE group_id = 0 LIMIT 1");
        if (!defaultPool.length || !defaultPool[0].values.length) {
          var defaultMirrorPath = require('path').join(__dirname, '..', 'files', 'default-mirror');
          try { require('fs').mkdirSync(defaultMirrorPath, { recursive: true }); } catch(e) {}
          db.run(
            "INSERT INTO storage_pools (local_path, group_id, mirror_index, status, sync_status, name) VALUES (?, 0, 0, 'active', 'synced', '默认镜像')",
            [defaultMirrorPath.replace(/\\/g, '/')]
          );
          console.log('[Init] 已创建默认镜像: ' + defaultMirrorPath + ' → 存储组#0');
        }
      } catch(e) { console.error('[Init] 创建默认镜像失败:', e.message); }
      ...
      `);
      // 兼容旧表：确保所有列存在
      safeAddColumn(db, 'backup_config', 'compress INTEGER DEFAULT 0');
      safeAddColumn(db, 'backup_config', 'webdav_enabled INTEGER DEFAULT 0');
      safeAddColumn(db, 'backup_config', 'webdav_url TEXT DEFAULT \'\'');
      safeAddColumn(db, 'backup_config', 'webdav_username TEXT DEFAULT \'\'');
      safeAddColumn(db, 'backup_config', 'webdav_password TEXT DEFAULT \'\'');
      safeAddColumn(db, 'backup_config', 'webdav_path TEXT DEFAULT \'/\'');
      safeAddColumn(db, 'backup_config', 'schedule_day INTEGER DEFAULT 0');
      safeAddColumn(db, 'backup_records', 'is_compressed INTEGER DEFAULT 0');
      safeAddColumn(db, 'backup_records', 'webdav_synced INTEGER DEFAULT 0');
      safeAddColumn(db, 'backup_records', 'webdav_message TEXT DEFAULT \'\'');

      // 首次建表时插入默认配置
      var existingCfg = db.exec("SELECT id FROM backup_config LIMIT 1");
      if (!existingCfg.length || !existingCfg[0].values.length) {
        var defaultBackupDir = path.join(__dirname, '..', 'backups').replace(/\\/g, '/');
        try { fs.mkdirSync(path.join(__dirname, '..', 'backups'), { recursive: true }); } catch(e) {}
        db.run('INSERT INTO backup_config (backup_dir) VALUES (?)', [defaultBackupDir]);
      }
      ...
      safeCreateTable(db, `
        CREATE TABLE IF NOT EXISTS backup_records (
          ...
          webdav_synced INTEGER DEFAULT 0,
          webdav_message TEXT DEFAULT '',
      ...
      `);
      try {
        db.run('CREATE INDEX IF NOT EXISTS idx_br_created ON backup_records(created_at)');
      } catch (e) {}


      // ==================== transfer_tasks — 文件传输任务表 ====================
      safeCreateTable(db, `
      ...
      `);
      try {
        db.run('CREATE INDEX IF NOT EXISTS idx_tt_user_status ON transfer_tasks(user_id, status)');
        db.run('CREATE INDEX IF NOT EXISTS idx_tt_transfer_id ON transfer_tasks(transfer_id)');
      } catch (e) {}

      // ==================== transfer_chunks — 传输分块表 ====================
      safeCreateTable(db, `
      ...
      `);
      try {
        db.run('CREATE UNIQUE INDEX IF NOT EXISTS idx_tc_task_chunk ON transfer_chunks(task_id, chunk_index)');
      } catch (e) {}
      ...
      `);
      try {
        db.run('CREATE INDEX IF NOT EXISTS idx_dl_user ON download_logs(user_id, created_at)');
      } catch (e) {}
...
const VirtualDir = {
  ...
  // 获取目录的完整路径（从根目录到当前目录，用 / 分隔）
  getFullPath: function(dirId) {
    var parts = [];
    var currentId = dirId;
    var maxDepth = 20;
    while (currentId && currentId > 0 && maxDepth-- > 0) {
      var dir = get('SELECT id, parent_id, name FROM virtual_dirs WHERE id = ?', [currentId]);
      if (!dir) break;
  ...
  // 全局搜索目录（跨目录 LIKE 匹配，仅当前用户，不限深度）
  searchByName: function(userId, keyword, limit) {
    limit = limit || 500;
    var likeQ = '%' + String(keyword).replace(/[%_\\]/g, '\\$&') + '%';
    return query(
      'SELECT id, name, parent_id, created_at FROM virtual_dirs ' +
      'WHERE user_id = ? AND name LIKE ? ESCAPE \'\\\' ' +
      'ORDER BY name LIMIT ' + Number(limit),
      [userId, likeQ]
    );
  },
...
const VirtualFile = {
  ...
  // 全局搜索文件（跨目录 LIKE 匹配，仅当前用户，不限深度）
  searchByName: function(userId, keyword, limit) {
    limit = limit || 500;
    var likeQ = '%' + String(keyword).replace(/[%_\\]/g, '\\$&') + '%';
    return query(
      'SELECT f.id, f.name, f.size, f.mime_type, f.dir_id, f.created_at, ' +
      'COALESCE(d.name, \'\') as dir_name ' +
      'FROM virtual_files f LEFT JOIN virtual_dirs d ON f.dir_id = d.id ' +
      'WHERE f.user_id = ? AND f.name LIKE ? ESCAPE \'\\\' ' +
      'ORDER BY f.name LIMIT ' + Number(limit),
      [userId, likeQ]
    );
  },
...
const RecycleBin = {
  ...
  moveFile: function(fileId, userId) {
    ...
    // 插入回收站记录（保留 enc_version + storage_id 用于永久删除时递减引用计数）
    run(
      'INSERT INTO deleted_files (user_id, dir_id, name, size, mime_type, storage_path, nonce, deleted_at, expires_at, original_dir_name, enc_version, storage_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [userId, file.dir_id, file.name, file.size, file.mime_type, file.storage_path, file.nonce, now, expires, dirName, file.enc_version || 0, file.storage_id || 0]
    );
  ...
  moveDir: function(dirId, userId) {
    ...
    allDirIds.forEach(function(did) {
      ...
      files.forEach(function(file) {
        // 插入文件回收站记录（dir_id=原虚拟目录id, recycle_dir_id=回收站目录id，保留 enc_version + storage_id）
        run(
          'INSERT INTO deleted_files (user_id, dir_id, name, size, mime_type, storage_path, nonce, deleted_at, expires_at, original_dir_name, recycle_dir_id, enc_version, storage_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
          [userId, did, file.name, file.size, file.mime_type, file.storage_path, file.nonce, now, expires, '', recycleIdMap[did], file.enc_version || 0, file.storage_id || 0]
        );
  ...
  // 内部：永久删除时确保存储引用计数正确递减
  _decrementStorageRef: function(file, userId) {
    if (!file.storage_id || file.storage_id <= 0) return;
    var UserFileRef = require('./db').UserFileRef;
    var FileStorage = require('./db').FileStorage;
    var existingRef = UserFileRef.findByUserAndFile(userId, file.storage_id);
    if (existingRef) {
      UserFileRef.removeOneRef(file.storage_id, userId, file.dir_id || 0, file.name);
      var newRef = FileStorage.decrementRef(file.storage_id);
      console.log('[RecycleBin] 永久删除修复: storage_id=' + file.storage_id + ' user=' + userId + ' ref_count→' + newRef);
      if (newRef <= 0) {
        // 引用为0，立即清理物理文件
        var paths = FileStorage.getValidPaths(file.storage_id);
        paths.forEach(function(p) {
          var fp = p.full_path;
          if (fp && !require('path').isAbsolute(fp) && p.local_path) fp = require('path').join(p.local_path, fp);
          try { require('fs').unlinkSync(fp); console.log('[RecycleBin] 物理删除: ' + fp); } catch(e) {}
        });
        FileStorage.delete(file.storage_id);
        console.log('[RecycleBin] ref_count=0, 已物理删除 storage_id=' + file.storage_id);
      }
    }
  },
  ...
  purgeFile: function(recycleFileId, userId) {
    var file = get('SELECT * FROM deleted_files WHERE id = ? AND user_id = ?', [recycleFileId, userId]);
    if (!file) return null;
    RecycleBin._decrementStorageRef(file, userId);
    run('DELETE FROM deleted_files WHERE id = ?', [recycleFileId]);
    return file;
  },
  ...
  emptyAll: function(userId) {
    ...
    allFiles.forEach(function(f) {
      ...
      RecycleBin._decrementStorageRef(f, userId);
    });
    ...
    expiredDirs.forEach(function(d) {
      ...
      allDirIds.forEach(function(rid) {
        ...
        dirFiles.forEach(function(f) {
          ...
          RecycleBin._decrementStorageRef(f, f.user_id);
...
var Share = {
  ...
  checkValidity: function(share) {
    if (!share) return { valid: false, reason: 'share_not_found' };
    if (share.disabled) return { valid: false, reason: 'disabled' };
    if (share.expires_at && new Date(share.expires_at) < new Date()) {
  ...
  // 切换禁用/启用状态（管理员可操作任意分享，用户可操作自己的）
  toggleDisabled: function(shareId, userId, isAdmin) {
    var share;
    if (isAdmin) {
      share = get('SELECT * FROM shares WHERE id = ?', [shareId]);
    } else {
      share = get('SELECT * FROM shares WHERE id = ? AND user_id = ?', [shareId, userId]);
    }
    if (!share) return { success: false, reason: 'not_found' };
    // 已过期则拒绝启用
    if (share.disabled && share.expires_at && new Date(share.expires_at) < new Date()) {
      return { success: false, reason: 'expired' };
    }
    var newVal = share.disabled ? 0 : 1;
    var disabledAt = newVal ? new Date().toISOString() : null;
    var disabledBy = newVal ? (isAdmin ? 'admin' : 'user') : null;
    run('UPDATE shares SET disabled = ?, disabled_at = ?, disabled_by = ? WHERE id = ?',
      [newVal, disabledAt, disabledBy, shareId]);
    return { success: true, disabled: !!newVal };
  },

  // 更新最后访问时间（公开分享被访问时调用）
  touchAccess: function(shareId) {
    run('UPDATE shares SET last_accessed_at = ? WHERE id = ?', [new Date().toISOString(), shareId]);
  },

  // 批量删除当前用户已过期的分享
  batchDeleteExpired: function(userId) {
    var now = new Date().toISOString();
    var result = run(
      'DELETE FROM shares WHERE user_id = ? AND expires_at IS NOT NULL AND expires_at < ?',
      [userId, now]
    );
    return (result.changes || 0);
  },

  // 查找不活跃分享（N天无访问，用于邮件告警）
  // daysWarning: 距今天天数阈值，last_accessed_at < 此阈值 或 NULL
  findInactiveShares: function(daysWarning) {
    var now = new Date();
    var warnThreshold = new Date(now.getTime() - daysWarning * 24 * 3600 * 1000).toISOString();
    return query(
      "SELECT s.*, u.email, u.nickname FROM shares s LEFT JOIN users u ON s.user_id = u.id WHERE s.disabled = 0 AND (s.expires_at IS NULL OR s.expires_at > ?) AND (s.last_accessed_at IS NULL OR s.last_accessed_at < ?) AND u.is_active = 1",
      [now.toISOString(), warnThreshold]
    );
  },

  // 查找需自动禁用的分享（N天无访问）
  findSharesForAutoDisable: function(daysInactive) {
    var now = new Date();
    var threshold = new Date(now.getTime() - daysInactive * 24 * 3600 * 1000).toISOString();
    return query(
      'SELECT * FROM shares WHERE disabled = 0 AND (expires_at IS NULL OR expires_at > ?) AND (last_accessed_at IS NULL OR last_accessed_at < ?)',
...
var RateLimitRules = {
  getAll: function() {
    return query('SELECT * FROM rate_limit_rules ORDER BY user_type, sort_order ASC', []);
  },

  getEnabled: function(userType) {
    return query(
      "SELECT * FROM rate_limit_rules WHERE user_type = ? AND is_enabled = 1 ORDER BY sort_order ASC",
      [userType]
    );
  },

  getById: function(id) {
    return get('SELECT * FROM rate_limit_rules WHERE id = ?', [id]);
  },

  add: function(userType, windowSeconds, maxRequests, banDurationSeconds, sortOrder) {
    var now = new Date().toISOString();
    var result = run(
      'INSERT INTO rate_limit_rules (user_type, window_seconds, max_requests, ban_duration_seconds, sort_order, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [userType, windowSeconds, maxRequests, banDurationSeconds, sortOrder || 0, now, now]
    );
    return result && result.lastInsertRowid ? result.lastInsertRowid : null;
  },
  ...
  update: function(id, fields) {
    var sets = [], params = [];
    var allowed = ['user_type', 'window_seconds', 'max_requests', 'ban_duration_seconds', 'is_enabled', 'sort_order'];
    ...
    params.push(id);
    return run('UPDATE rate_limit_rules SET ' + sets.join(', ') + ' WHERE id = ?', params);
  },

  delete: function(id) {
    return run('DELETE FROM rate_limit_rules WHERE id = ?', [id]);
  }
};

// ==================== 频率限制白名单 ====================
var RateLimitWhitelist = {
  getAll: function() {
    return query('SELECT * FROM rate_limit_whitelist ORDER BY path ASC', []);
  },

  getEnabled: function() {
    return query("SELECT * FROM rate_limit_whitelist WHERE is_enabled = 1 ORDER BY path ASC", []);
  },

  add: function(path, description) {
    var now = new Date().toISOString();
    var result = run(
      'INSERT INTO rate_limit_whitelist (path, description, created_at) VALUES (?, ?, ?)',
  ...
  update: function(id, fields) {
    var sets = [], params = [];
    var allowed = ['path', 'description', 'is_enabled'];
    allowed.forEach(function(f) {
      if (fields[f] !== undefined) {
        sets.push(f + ' = ?');
        params.push(fields[f]);
    ...
    if (!sets.length) return false;
    params.push(id);
    return run('UPDATE rate_limit_whitelist SET ' + sets.join(', ') + ' WHERE id = ?', params);
  },

  delete: function(id) {
    return run('DELETE FROM rate_limit_whitelist WHERE id = ?', [id]);
  }
};
...
var TrafficLog = {
  // 记录一条流量（由上层函数批量缓冲）
  log: function(userId, guestIp, actionType, fileId, fileName, fileSize, bytesCount, trafficCategory) {
    var now = new Date().toISOString();
    run(
      'INSERT INTO traffic_logs (user_id, guest_ip, action_type, file_id, file_name, file_size, bytes_count, traffic_category, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [userId || 0, guestIp || '', actionType, fileId || 0, fileName || '', fileSize || 0, bytesCount || 0, trafficCategory || 'file_transfer', now]
    );
  },
  ...
  logBatch: function(records) {
    try {
      records.forEach(function(r) {
        TrafficLog.log(r.user_id, r.guest_ip, r.action_type, r.file_id, r.file_name, r.file_size, r.bytes_count, r.traffic_category);
      });
    } catch(e) {}
  },
  ...
  list: function(opts) {
    ...
    if (opts.traffic_category) { where.push('tl.traffic_category = ?'); params.push(opts.traffic_category); }
...
var WebDAVLink = {
  create: function(userId, targetPath, targetName, isDirectory, expiresDays, requireAuth, targetType) {
    ...
    run(
      'INSERT INTO webdav_links (user_id, token, target_path, target_name, is_directory, expires_at, created_at, last_accessed, require_auth, target_type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [userId, token, targetPath, targetName, isDirectory ? 1 : 0, expiresAt, now.toISOString(), now.toISOString(), requireAuth ? 1 : 0, targetType || 'public']
    );
  ...
  // 切换禁用/启用状态
  toggleDisabled: function(id, userId, isAdmin) {
    var link;
    if (isAdmin) {
      link = get('SELECT * FROM webdav_links WHERE id = ?', [id]);
    } else {
      link = get('SELECT * FROM webdav_links WHERE id = ? AND user_id = ?', [id, userId]);
    }
    ...
    var newVal = link.disabled ? 0 : 1;
    var disabledAt = newVal ? new Date().toISOString() : null;
    var disabledBy = newVal ? (isAdmin ? 'admin' : 'user') : null;
    run('UPDATE webdav_links SET disabled = ?, disabled_at = ?, disabled_by = ? WHERE id = ?',
      [newVal, disabledAt, disabledBy, id]);
    return { success: true, disabled: !!newVal };
  },

  // 延长有效期：到期前30天内可续期最多1年（从今天起）
  extend: function(token, userId) {
    var link = get('SELECT * FROM webdav_links WHERE token = ? AND user_id = ?', [token, userId]);
    if (!link) return { success: false, reason: 'not_found' };
    if (!link.expires_at) return { success: false, reason: 'permanent_link' };
    var now = new Date();
    var currentExpiry = new Date(link.expires_at);
    var oneMonthFromNow = new Date(now.getTime() + 30 * 24 * 3600 * 1000);
    if (currentExpiry > oneMonthFromNow) {
      return { success: false, reason: 'not_near_expiry', remaining_days: Math.ceil((currentExpiry - now) / (24 * 3600 * 1000)) };
    }
    var newExpiry = new Date(now.getTime() + 365 * 24 * 3600 * 1000).toISOString();
    run('UPDATE webdav_links SET expires_at = ? WHERE id = ?', [newExpiry, link.id]);
    return { success: true, new_expires_at: newExpiry };
  },

  // 管理员列表（token加密返回）
  listAllAdmin: function(page, limit, userId, keyword) {
    var offset = (page - 1) * limit;
    var params = [];
    var where = [];
    if (userId > 0) {
    ...
    if (keyword) {
      where.push('(w.target_name LIKE ? OR u.email LIKE ? OR w.target_path LIKE ? OR w.link_name LIKE ?)');
      var k = '%' + keyword + '%';
      params.push(k, k, k, k);
    }
    var whereStr = where.length > 0 ? 'WHERE ' + where.join(' AND ') : '';
    var links = query(
      'SELECT w.*, u.email as owner_email, u.nickname as owner_nickname FROM webdav_links w LEFT JOIN users u ON w.user_id = u.id ' + whereStr + ' ORDER BY w.created_at DESC LIMIT ' + limit + ' OFFSET ' + offset,
      params
    );
    var total = get('SELECT COUNT(*) as count FROM webdav_links w LEFT JOIN users u ON w.user_id = u.id ' + whereStr, params);
    var now = new Date().toISOString();
    var result = links.map(function(l) {
      l.is_expired = !!(l.expires_at && l.expires_at < now);
      l.disabled = !!l.disabled;
      // 保存 token 供管理员操作使用（不在 UI 中显示完整 token）
      l._tid = l.token;
      // 加密隐藏 token 中间部分，格式: 前8 + •••• + 后8
      var tok = l.token || '';
      if (tok.length > 20) {
        l.masked_token = tok.substring(0, 8) + '••••••••••••' + tok.substring(tok.length - 8);
      } else if (tok.length > 8) {
        l.masked_token = tok.substring(0, 4) + '••••••••••••' + tok.substring(tok.length - 4);
      } else {
        l.masked_token = tok;
      }
      // 不返回完整 token 给管理员前端
      delete l.token;
      // 路径：个人链接 target_path 存的是目录 ID，用 target_name 代替
      if (l.target_type === 'personal') {
        l.display_path = l.target_name || l.target_path || '/';
      } else {
        var rawPath = l.target_path || l.target_name || '';
        l.display_path = (rawPath && rawPath[0] !== '/') ? '/' + rawPath : (rawPath || '/');
      }
      return l;
    });
    return { links: result, total: total ? total.count : 0 };
  },
  ...
  batchDeleteExpired: function(userId) {
    var now = new Date().toISOString();
    var result = run(
      'DELETE FROM webdav_links WHERE user_id = ? AND expires_at IS NOT NULL AND expires_at < ?',
      [userId, now]
    );
    return (result.changes || 0);
  },

  // 查找不活跃链接（N天无访问，用于邮件告警）
  findInactiveLinks: function(daysInactive, daysWarning) {
    var now = new Date();
    var warnThreshold = new Date(now.getTime() - daysWarning * 24 * 3600 * 1000).toISOString();
    var inactiveThreshold = new Date(now.getTime() - daysInactive * 24 * 3600 * 1000).toISOString();
    return query(
      "SELECT w.*, u.email, u.nickname FROM webdav_links w LEFT JOIN users u ON w.user_id = u.id WHERE w.disabled = 0 AND (w.expires_at IS NULL OR w.expires_at > ?) AND (w.last_accessed IS NULL OR w.last_accessed < ?) AND (w.last_accessed IS NULL OR w.last_accessed >= ?) AND u.is_active = 1",
      [now.toISOString(), warnThreshold, inactiveThreshold]
    );
  },

  // 查找需自动禁用的链接（N天无访问）
  findLinksForAutoDisable: function(daysInactive) {
    var now = new Date();
    var threshold = new Date(now.getTime() - daysInactive * 24 * 3600 * 1000).toISOString();
    return query(
      'SELECT * FROM webdav_links WHERE disabled = 0 AND (expires_at IS NULL OR expires_at > ?) AND (last_accessed IS NULL OR last_accessed < ?)',
      [now.toISOString(), threshold]
    );
  }
};
...
var FileStorage = {
  ...
  listRefUsers: function(storageId) {
    return query(
      'SELECT ufr.id, ufr.user_id, ufr.dir_id, ufr.name, u.email FROM user_file_refs ufr LEFT JOIN users u ON ufr.user_id = u.id WHERE ufr.storage_id = ?',
      [storageId]
    );
  },
...
var UserFileRef = {
  ...
  // 通过 storage_id + user_id 删除引用（可能删除多条！精确删除请用 removeOneRef）
  ...
  // 精确删除一条引用（按 storage + user + dir + name 匹配）
  removeOneRef: function(storageId, userId, dirId, name) {
    var ref = get('SELECT * FROM user_file_refs WHERE storage_id = ? AND user_id = ? AND dir_id = ? AND name = ?',
      [storageId, userId, dirId || 0, name]);
    if (ref) {
      run('DELETE FROM user_file_refs WHERE storage_id = ? AND user_id = ? AND dir_id = ? AND name = ?',
        [storageId, userId, dirId || 0, name]);
    }
    return ref;
  },
...
var BackupConfig = {
  get: function() {
    var row = get('SELECT * FROM backup_config ORDER BY id LIMIT 1');
    if (!row) return null;
    // 防御性：确保所有字段存在（即使表结构不完整）
    return {
      id: row.id || 0,
      enabled: !!(row.enabled),
      schedule_type: row.schedule_type || 'daily',
      schedule_time: row.schedule_time || '03:00',
      schedule_day: parseInt(row.schedule_day, 10) || 0,
      backup_dir: row.backup_dir || '',
      retention_days: parseInt(row.retention_days, 10) || 30,
      compress: !!(row.compress),
      webdav_enabled: !!(row.webdav_enabled),
      webdav_url: row.webdav_url || '',
      webdav_username: row.webdav_username || '',
      webdav_password: row.webdav_password || '',
      webdav_path: row.webdav_path || '/',
      updated_at: row.updated_at || ''
    };
  },
  save: function(opts) {
    var existing = BackupConfig.get();
    if (existing) {
      run(
        'UPDATE backup_config SET enabled=?, schedule_type=?, schedule_time=?, schedule_day=?, backup_dir=?, retention_days=?, compress=?, webdav_enabled=?, webdav_url=?, webdav_username=?, webdav_password=?, webdav_path=?, updated_at=datetime(\'now\') WHERE id=?',
        [
          opts.enabled ? 1 : 0,
          opts.schedule_type || 'daily',
          opts.schedule_time || '03:00',
          opts.schedule_day || 0,
          opts.backup_dir || '',
          opts.retention_days || 30,
          opts.compress ? 1 : 0,
          opts.webdav_enabled ? 1 : 0,
          opts.webdav_url || '',
          opts.webdav_username || '',
          opts.webdav_password || '',
          opts.webdav_path || '/',
          existing.id
        ]
      );
      ...
      run(
        'INSERT INTO backup_config (enabled, schedule_type, schedule_time, schedule_day, backup_dir, retention_days, compress, webdav_enabled, webdav_url, webdav_username, webdav_password, webdav_path) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)',
        [
          opts.enabled ? 1 : 0,
          opts.schedule_type || 'daily',
          opts.schedule_time || '03:00',
...
var BackupRecord = {
  create: function(data) {
    var result = run(
      'INSERT INTO backup_records (filename, file_path, file_size, is_compressed, webdav_synced, webdav_message, status, error_message, task_id) VALUES (?,?,?,?,?,?,?,?,?)',
      [
        data.filename || '',
        data.file_path || '',
        data.file_size || 0,
        data.is_compressed ? 1 : 0,
        data.webdav_synced || 0,
        data.webdav_message || '',
  ...
  get: function(id) {
    return get('SELECT * FROM backup_records WHERE id = ?', [id]);
  },
  ...
  count: function() {
    var row = get('SELECT COUNT(*) as cnt FROM backup_records');
    return row ? row.cnt : 0;
  },
  totalSize: function() {
    var row = get('SELECT COALESCE(SUM(file_size), 0) as total FROM backup_records');
    return row ? row.total : 0;
  },
  last: function() {
    return get('SELECT * FROM backup_records ORDER BY created_at DESC LIMIT 1');
  },
  delete: function(id) {
    return run('DELETE FROM backup_records WHERE id = ?', [id]);
  },
  deleteOlderThan: function(cutoffDate) {
    var oldRecords = query('SELECT * FROM backup_records WHERE created_at < ?', [cutoffDate]);
    oldRecords.forEach(function(r) {
      try { fs.unlinkSync(r.file_path); } catch(e) {}
      run('DELETE FROM backup_records WHERE id = ?', [r.id]);
    });
...
// ==================== TransferTask — 文件传输任务模型 ====================
var TransferTask = {
  create: function(data) {
    var result = run(
      'INSERT INTO transfer_tasks (user_id, direction, transfer_id, file_name, file_size, mime_type, file_hash, dir_id, chunk_size, total_chunks, device_id, device_name, ip, status, started_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
      [data.user_id || 0, data.direction || 'upload', data.transfer_id || '', data.file_name || '', data.file_size || 0,
       data.mime_type || 'application/octet-stream', data.file_hash || '', data.dir_id || 0,
       data.chunk_size || 4194304, data.total_chunks || 0, data.device_id || '', data.device_name || '',
       data.ip || '', data.status || 'uploading', new Date().toISOString()]
    );
    return result.lastInsertRowid;
  },
  findByTransferId: function(transferId) {
    return get('SELECT * FROM transfer_tasks WHERE transfer_id = ?', [transferId]);
  },
  findById: function(id) {
    return get('SELECT * FROM transfer_tasks WHERE id = ?', [id]);
  },
  listByUser: function(userId, status, limit, offset) {
    var sql = 'SELECT * FROM transfer_tasks WHERE user_id = ?';
    var params = [userId];
    if (status && status !== 'all') { sql += ' AND status = ?'; params.push(status); }
    sql += ' ORDER BY updated_at DESC LIMIT ? OFFSET ?';
    params.push(limit || 30, offset || 0);
    return query(sql, params);
  },
  countByUser: function(userId, status) {
    var sql = 'SELECT COUNT(*) as cnt FROM transfer_tasks WHERE user_id = ?';
    var params = [userId];
    if (status && status !== 'all') { sql += ' AND status = ?'; params.push(status); }
    var row = get(sql, params);
    return row ? row.cnt : 0;
  },
  findPendingByUser: function(userId) {
    return query(
      "SELECT * FROM transfer_tasks WHERE user_id = ? AND status IN ('pending','uploading','paused','assembling') ORDER BY updated_at DESC",
      [userId]
    );
  },
  updateStatus: function(id, status, errorMessage) {
    return run(
      'UPDATE transfer_tasks SET status = ?, error_message = ?, updated_at = ? WHERE id = ?',
      [status, errorMessage || '', new Date().toISOString(), id]
    );
  },
  updateProgress: function(id, uploadedChunks, totalChunks) {
    var progress = totalChunks > 0 ? Math.floor(uploadedChunks / totalChunks * 100) : 0;
    return run(
      'UPDATE transfer_tasks SET uploaded_chunks = ?, total_chunks = ?, progress = ?, updated_at = ? WHERE id = ?',
      [uploadedChunks, totalChunks, progress, new Date().toISOString(), id]
    );
  },
  completeTask: function(id, storagePath, fileHash, virtualFileId) {
    return run(
      "UPDATE transfer_tasks SET status = 'completed', storage_path = ?, file_hash = ?, metadata = ?, completed_at = ?, updated_at = ?, progress = 100 WHERE id = ?",
      [storagePath || '', fileHash || '', JSON.stringify({ virtual_file_id: virtualFileId || 0 }), new Date().toISOString(), new Date().toISOString(), id]
    );
  },
  setAssembling: function(id) {
    return run("UPDATE transfer_tasks SET status = 'assembling', updated_at = ? WHERE id = ?",
      [new Date().toISOString(), id]);
  },
  cancelTask: function(id) {
    return run("UPDATE transfer_tasks SET status = 'cancelled', updated_at = ? WHERE id = ?",
      [new Date().toISOString(), id]);
  },
  delete: function(id) {
    return run('DELETE FROM transfer_tasks WHERE id = ?', [id]);
  },
  cleanup: function(days) {
    var cutoff = new Date();
    cutoff.setDate(cutoff.getDate() - (days || 7));
    var oldTasks = query(
      "SELECT id, transfer_id FROM transfer_tasks WHERE status IN ('completed','error','cancelled') AND updated_at < ?",
      [cutoff.toISOString()]
    );
    oldTasks.forEach(function(t) {
      try {
        var chunkDir = require('path').join(require('path').dirname(require('./db').getChunkDir()), t.transfer_id);
        require('fs').rmSync(chunkDir, { recursive: true, force: true });
      } catch(e) {}
      run('DELETE FROM transfer_tasks WHERE id = ?', [t.id]);
    });
    return oldTasks.length;
  }
};

// ==================== TransferChunk — 传输分块模型 ====================
var TransferChunk = {
  upsert: function(taskId, chunkIndex, chunkSize, chunkPath) {
    var existing = get('SELECT id FROM transfer_chunks WHERE task_id = ? AND chunk_index = ?', [taskId, chunkIndex]);
    if (existing) {
      return run("UPDATE transfer_chunks SET chunk_size = ?, chunk_path = ?, status = 'uploaded', uploaded_at = ? WHERE id = ?",
        [chunkSize, chunkPath, new Date().toISOString(), existing.id]);
    } else {
      return run("INSERT INTO transfer_chunks (task_id, chunk_index, chunk_size, chunk_path, status, uploaded_at) VALUES (?,?,?,?,'uploaded',?)",
        [taskId, chunkIndex, chunkSize, chunkPath, new Date().toISOString()]);
    }
  },
  findByTask: function(taskId) {
    return query('SELECT * FROM transfer_chunks WHERE task_id = ? ORDER BY chunk_index', [taskId]);
  },
  countUploaded: function(taskId) {
    var row = get("SELECT COUNT(*) as cnt FROM transfer_chunks WHERE task_id = ? AND status = 'uploaded'", [taskId]);
    return row ? row.cnt : 0;
  },
  deleteByTask: function(taskId) {
    return run('DELETE FROM transfer_chunks WHERE task_id = ?', [taskId]);
  }
};
...
var DownloadLog = {
  log: function(data) {
    return run(
      'INSERT INTO download_logs (user_id, file_id, file_name, file_size, device_id, device_name, ip, user_agent, status, error_message) VALUES (?,?,?,?,?,?,?,?,?,?)',
      [data.user_id || 0, data.file_id || 0, data.file_name || '', data.file_size || 0,
       data.device_id || '', data.device_name || '', data.ip || '', data.user_agent || '',
       data.status || 'completed', data.error_message || '']
    );
  },
  listByUser: function(userId, limit, offset) {
    return query(
  ...
  countByUser: function(userId) {
    var row = get('SELECT COUNT(*) as cnt FROM download_logs WHERE user_id = ?', [userId]);
    return row ? row.cnt : 0;
  },
  listRecent: function(limit) {
    return query('SELECT * FROM download_logs ORDER BY created_at DESC LIMIT ?', [limit || 20]);
  }
};
